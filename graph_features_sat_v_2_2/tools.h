...
#include <math.h>
#include <cmath> // Para std::std::abs
#include <vector>
#include <map>
#include <assert.h>
#include <stdio.h>
...
#define MAXITER 10000
  ...
  if (xmin < 25) {
    ...
    while (std::abs(p - pold) > 0.00000001 && n < MAXITER) {
  ...
  //       the best alpha, xmin, dif and where is located--------------------
    ...
    //------------- Model powerlaw -----------------------------------------
		...
		for (int j=ind+1; j<n; j++) {
			double aux;
			aux = std::abs((double)y[j]/y[ind] - powlawc((int)x[j],xmin,alpha));
			if (aux >= bestdifa) {
				worstdif = aux;
				worstx = (int)x[j]; 
		...
		for (int j=ind; j<n; j++) {
			if (x[j] + 1 < x[j+1]) {
				double aux;
				aux = std::abs((double)y[j+1]/y[ind] - powlawc((int)x[j]+1,xmin,alpha));
				if (aux >= bestdifa) {
					worstdif = aux;
					worstx = (int)x[j]+1; 
					j = n;  //Finish search of worst diff
		...
		//------------- Model exponential -----------------------------------------
			...
			for (int j=ind+1; j<n; j++) {
				double aux;
				aux = std::abs((double)y[j]/y[ind] - exponc(x[j],xmin,beta));
				if (aux >= bestdifb) {
					worstdif = aux;
					worstx = x[j]; 
					j = n;  //Finish search of worst diff
			...
			for (int j=ind; j<n; j++) {
				if (x[j] + 1 < x[j+1]) {
					double aux;
					aux = std::abs((double)y[j+1]/y[ind] - exponc(x[j]+1,xmin,beta));
					if (aux >= bestdifb) {
						worstdif = aux;
						worstx = x[j]+1; 
						j = n;  //Finish search of worst diff
...
//--------------------------------------------------------------------------------	
	...
	while(fscanf(source, "%i", &var)==1) {
		if (var==0) {
			...
			if (std::abs(var) > totVars) {
				cerr << "Unvalid variable number " << std::abs(var) << endl;
				exit(-1);
			}
			clause.push_back(std::abs(var)-1);
		}
	}
