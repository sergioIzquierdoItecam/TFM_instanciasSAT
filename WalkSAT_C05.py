...
class WalkSAT:
    ...
    def __init__(self, variables, clauses, clauseLength, seed, modularity, communities):
        ...
        self.formula, self.communities_variables, self.variable_to_community, self.clause_community_count,self.clause_to_community = self.generate_random_model()
    ...
    def generate_random_model(self):
        ...
        for clause in formula:
            ...
            # Recorrer todas las cláusulas
            for clause_index, clause in enumerate(formula):
                # Obtener las comunidades de las variables en la cláusula
                communities = set()
                for var in clause:
                    var_abs = abs(var)
                    if var_abs in variable_to_community:
                        communities.add(variable_to_community[var_abs])
        
                # Si todas las variables en la cláusula pertenecen a la misma comunidad
                if len(communities) == 1:
                    # Obtener la comunidad de la cláusula
                    community = communities.pop()
                    clause_to_community[clause_index + 1] = community
            
        return formula, communities_variables, variable_to_community, clause_community_count, clause_to_community
    ...
    def solve(self, max_flips, max_tries, probability):
        ...
        for tries in range(max_tries):
            ...
            for flips in range(max_flips):
                ...
                community_unsatisfied_clauses = {community: 0 for community in set(self.variable_to_community.values())}
                
                for clause in clauses_unsatisfied:
                    if clause in self.clause_to_community:
                        community_unsatisfied_clauses[self.clause_to_community[clause]] += 1
                        
                while(True):                    
                    community_max = max(community_unsatisfied_clauses, key=community_unsatisfied_clauses.get)
                    variables_max = [key for key, value in self.variable_to_community.items() if value == community_max]
                    variable = random.choice(variables_max)  
