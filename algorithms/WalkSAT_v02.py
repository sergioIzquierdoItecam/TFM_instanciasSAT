    ...
    def solve(self, max_flips, max_tries, probability):
        try:
            for tries in range(max_tries):
                ...
                # Flip initial variables in communities with only one community
                if clauses_unsatisfied_one_community:
                    communities_flipped = []  
                    variables_flipped = [] 
                    count_variables_flipped = {}                     
                    for clause in clauses_unsatisfied_one_community:
                        variable = random.choice(self.formula[clause-1])
                        community = self.variable_to_community[abs(variable)]
                        # Skip if we've already flipped this variable
                        if abs(variable) in variables_flipped:
                            continue

                        count_variables_flipped[abs(variable)] = count_variables_flipped.get(abs(variable), 0) + 1

                        variables_flipped.append(abs(variable))
                        # Flip the variable
                        assignment[abs(variable)] = not assignment[abs(variable)]

                        # # Check if this initial flip solved the problem
                    variable_clauses, score_clauses, clause_assignment = self.get_variable_clauses(assignment)
                    satisfied_total = sum(1 for score in score_clauses.values() if score != 0)
            ...
            traceback.print_exc()  # Imprime el traceback completo (incluyendo la línea del error)
            print(f"Error: {e}")
